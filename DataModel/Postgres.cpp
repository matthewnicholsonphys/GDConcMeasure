...
//////////////////////////
// The following methods should probably be in another location, perhaps another helper class.


bool Postgres::GetCurrentRun(int& runnum, int* runconfig, std::string* err){
	...
	get_ok = ExecuteQuery(query_string, runnum);
	if(not get_ok){
	...
	if(runconfig){
		...
		get_ok = ExecuteQuery(query_string, *runconfig);
		if(not get_ok){
...
bool Postgres::GetRunConfig(int& runconfig, int* runnum_in, std::string* err){
...
bool Postgres::GetToolsConfig(std::string& toolsconfig, int* runnum, int* runconfig_in, std::string* err){
	...
	get_ok = ExecuteQuery(query_string, toolsconfig);
...
bool Postgres::GetToolConfig(std::string toolname, std::string& toolconfig, int* versionnum_in, int* runconfig, int* runnum, std::string* err){
	...
	if(*versionnum<0){
		...
		if(not toolsconfigstore.Get(toolname,*versionnum)){
			std::string errmsg = "Postgres::GetToolConfig error! Tool "+toolname
		...
		// sanity check
		if(*versionnum<0){
			std::string errmsg = "Postgres::GetToolConfig error! invalid version number "
	...
	std::string query_string = "SELECT contents FROM configfiles WHERE tool = "+pqxx_quote_name(toolname)
	...
	get_ok = ExecuteQuery(query_string, toolconfig);
	if(not get_ok){
